...
def validate_raw_data(rides: pd.DataFrame, year:int,month:int) -> pd.DataFrame:
    "This function remove rows with pickup_datetimes outside their valid range"
...
def load_raw_data(
    ...
    if months is None:
        #download data only for the moths specified by 'months'
    ...
    for month in months:
        ...
        if not local_file.exists():
            ...
            print(f'File {year}-{month:02d} already in the local_storage') 
